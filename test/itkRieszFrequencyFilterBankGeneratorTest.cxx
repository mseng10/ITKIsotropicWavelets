...
itkRieszFrequencyFilterBankGeneratorTest( int argc, char* argv[] )
{
  ...
  if ( orderIsEven )
    ...
    for ( unsigned int comp = 0; comp < filterBank->GetNumberOfOutputs(); comp++ )
      ...
      itk::ViewImage<ImageType>::View( complexToRealFilter->GetOutput(), "RealPart of Complex. Component: " + n2s(
...
#ifdef ITK_VISUALIZE_TESTS
      itk::NumberToString< unsigned int > n2s;
      itk::ViewImage<ImageType>::View( complexToImaginaryFilter->GetOutput(), "ImaginaryPart of Complex. Component: " + n2s(
          comp ) + " / " + n2s( filterBank->GetNumberOfOutputs() - 1 ) + ". Indices: " + oss.str() );
#endif
      }
    }
