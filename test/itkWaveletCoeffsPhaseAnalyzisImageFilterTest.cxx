...
itkWaveletCoeffsPhaseAnalyzisImageFilterTest( int argc, char *argv[] )
{
  if ( argc < 8 || argc > 9 )
    ...
    std::cerr << argv[0] << " inputImageFile outputImageFile inputLevels inputBands waveletFunction dimension Apply|NoApply [thresholdNumOfSigmas]" << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if ( waveletFunction == "Held" )
    ...
    using WaveletType = itk::HeldIsotropicWavelet< WaveletScalarType, ImageDimension>;
      ...
      auto waveletCoeffsPhaseAnalyzisImageFilter = WaveletCoeffsPhaseAnalyzisImageFilterType::New();
      EXERCISE_BASIC_OBJECT_METHODS( waveletCoeffsPhaseAnalyzisImageFilter, WaveletCoeffsPhaseAnalyzisImageFilter,
        ImageToImageFilter );

      return runWaveletCoeffsPhaseAnalyzisImageFilterTest< 3, WaveletType >(  inputImage,
        outputImage,
        inputLevels,
        inputBands,
        applySoftThreshold,
